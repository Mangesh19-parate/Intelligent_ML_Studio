...
**ADR-017: Adopt the full granular sub-stage taxonomy; retain "Diagnostics" over "Intelligence."**
The project owner specified a complete sub-stage breakdown for all 8 stages, including a stage labeled "Intelligence." This directly named the term ADR-002 retired after multiple review rounds specifically to avoid AI-hype framing. Resolved: "Diagnostics" remains the canonical stage name (fit diagnosis, recommendations, explainability all live under it); "Intelligence" is treated as an informal label for the same stage rather than a reversal of ADR-002. If the project owner wants to formally reverse ADR-002, that requires an explicit new decision, not an incidental one — flagged rather than silently applied.

**ADR-018: "Recommender system" resolved to mean the existing Recommendations engine, not a new ML paradigm.**
A literal recommender system (collaborative filtering, content-based recommendation) operates on user-item interaction data, which doesn't exist in this platform's domain. Building one would be a large, unrelated scope addition to a tabular regression/classification governance platform. Resolved: "recommender system" in the taxonomy maps to the existing evidence-based `recommendations` engine (finding/evidence/action/risk/confidence), already specified since the earliest SRS drafts.

**ADR-019: Four genuinely new capabilities added — Mixed-Variable Handling, Date/Time Handling, Feature Construction, Feature Extraction — each classified explicitly for fold-scoping.**
The requested sub-stage taxonomy named several sub-items that map cleanly onto existing capabilities (missing values, categorical encoding, feature scaling, feature selection) and four that don't exist yet. Rather than treat all four identically, each was individually classified against Invariant 1: mixed-variable handling and feature construction are deterministic arithmetic/parsing with no fold-scoping requirement; date/time reference statistics and feature extraction (PCA) are learned parameters requiring the same fold-scoped treatment as imputation and scaling. Treating a deterministic operation as needing fold-scoping would be unnecessary complexity; treating a learned one as if it were deterministic would reproduce the exact leakage bug this project exists to prevent. See SRS v11 §2, §3, §6.

**ADR-020: "Register" and "Pipeline Validation" are naming clarifications only — no new scope.**
"Register" (Machine Learning) already exists as the Model Registry (`trained_models`, artifact storage, Model Passport). "Pipeline Validation" (Production) already exists as the Deployment Gate (8a/8b split). Both are explicitly named in the taxonomy and in user-facing labels going forward; neither required new backend capability.

...
export const LandingPage: React.FC = () => {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setAuthInfoMessage(res.data?.message || 'A fresh 6-digit code was sent to your email.');
    ...
    try {
      ...
      if (isRegister) {
        ...
        if (res.requires_2fa && res.two_factor_token) {
          ...
          setMaskedEmail(res.email_masked || authEmail);
          setIs2FAPrompt(true);
          setTwoFactorCode('');
          setResendCooldown(30);
        } else {
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              {is2FAPrompt ? (
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col items-center space-y-2 text-xs pt-1">
                    <button
                      type="button"
                      onClick={handleResendOtp}
                      disabled={resendCooldown > 0 || resending}
                      className="text-xs text-[var(--color-accent)] hover:underline font-semibold flex items-center space-x-1.5 cursor-pointer disabled:opacity-50 disabled:no-underline"
                    >
